...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores_3, cache_3=affine_forward(scores_2, self.params['W2'], self.params['b2'])
    scores=scores_3
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dx3, dw2, db2=affine_backward(dx0, cache_3)
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    while i<= self.num_layers:
        ...
        if i==self.num_layers:
            self.params[weight]=weight_scale*np.random.randn(hidden_dims[i-2], num_classes)
            self.params[bias]=np.zeros(num_classes)
        elif i==1:
            self.params[weight]=weight_scale*np.random.randn(input_dim, hidden_dims[i-1])
            ...
            self.params[weight]=weight_scale*np.random.randn(hidden_dims[i-2], hidden_dims[i-1])
            self.params[bias]=np.zeros(hidden_dims[i-1])
            if self.use_batchnorm:
                self.params[gamma]=np.ones(hidden_dims[i-1])
                self.params[beta]=np.zeros(hidden_dims[i-1])
  ...
  def loss(self, X, y=None):
    ...
    while i<=self.num_layers:
        ...
        if i==self.num_layers:
            ...
            scores, d_affine_cache[index]=affine_forward(X, self.params[weight], self.params[bias])
            if self.use_batchnorm:
            ...
            if self.use_dropout:
                scores, d_drop_cache[index]=dropout_forward(scores, self.dropout_param)
        else:
            scores, d_affine_cache[index]=affine_forward(scores, self.params[weight], self.params[bias])
            if self.use_batchnorm:
                scores, d_bn_cache[index]=batchnorm_forward(scores, self.params[gamma], self.params[beta], self.bn_params[i-1])
    ...
    while j>0:
        ...
        if j==self.num_layers:
            ...
            if self.use_dropout:
                dx=dropout_backward(dx, d_drop_cache[index])
